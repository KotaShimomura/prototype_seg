...
from lib.datasets.loader.default_loader import CSDataTestLoader, DefaultLoader
from lib.datasets.tools.collate import collate
from lib.utils.distributed import get_rank, get_world_size, is_distributed
...
class DataLoader(object):
    def __init__(self, configer):
        ...
        self.aug_val_transform = cv2_aug_transforms.CV2AugCompose(
    ...
    def get_dataloader_sampler(self, klass, split, dataset):
        ...
        from lib.datasets.loader.multi_dataset_loader import (
            MultiDatasetLoader,
        ...
        kwargs = dict(
            dataset=dataset,
            aug_transform=(
                self.aug_train_transform if split == "train" else self.aug_val_transform
            ),
        ...
        if split == "train":
            if is_distributed() and multi_dataset:
                raise RuntimeError(
                    "Currently multi dataset doesn't support distributed."
                )
    ...
    def get_trainloader(self):
        if (
            self.configer.exists("data", "use_edge")
            and self.configer.get("data", "use_edge") == "ce2p"
        ):
            """
            ce2p manner:
        ...
        elif self.configer.exists("data", "use_dt_offset") or self.configer.exists(
            "data", "pred_dt_offset"
        ):
            """
            dt-offset manner:
    ...
    def get_valloader(self, dataset=None):
        ...
        if self.configer.exists("data", "use_dt_offset") or self.configer.exists(
            "data", "pred_dt_offset"
        ):
            """
            dt-offset manner:
            ...
            Log.error(
                "Method: {} loader is invalid.".format(self.configer.get("method"))
            )
            return None
        ...
        valloader = data.DataLoader(
            loader,
            sampler=sampler,
            batch_size=self.configer.get("val", "batch_size") // get_world_size(),
            pin_memory=False,
    ...
    def get_testloader(self, dataset=None):
        ...
        if self.configer.exists("data", "use_sw_offset") or self.configer.exists(
            "data", "pred_sw_offset"
        ):
