        ...
        def _set_value(key, value):
            ...
            if cur_key.endswith("+"):
                ...
                if not isinstance(target, list):
                    Log.error(
                        "Cannot append to {key}, as its type is {target_type}.".format(
                            key=key[:-1], target_type=type(target)
                        )
                    )
                    sys.exit(1)
                ...
                target.append(value)
                Log.info(
                    "Append {value} to {key}. Current: {target}.".format(
                        key=key[:-1],
                        value=value,
                        target=target,
                    )
            ...
            if existing_value is not None:
                Log.warn(
                    "Override {key} using {value}. Previous value: {old_value}.".format(
                        key=key, value=value, old_value=existing_value
                    )
                )
            else:
...
class _ConditionHelper:
    ...
    def diverse_size(self):
        return (
            self.configer.get("val", "data_transformer")["size_mode"] == "diverse_size"
        )
